...
export const getMyTasks = async () => {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/tasks/my-tasks`, {
      method: "GET",
      headers: {
...
export const updateTaskStatus = async (id: string, status: string, note?: string) => {
  const storeCookie = await cookies();
  const token = storeCookie.get("token")?.value;

  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/tasks/${id}/status`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`
