  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-black dark:text-white mb-2">Task Disputes</h1>
          <p className="text-muted-foreground text-lg font-medium">Manage and track cases where issues were reported on your tasks.</p>
        </div>
        <div className="px-6 py-3 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-600 font-bold flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 gap-6">
          ...
          >
            <div className="flex flex-col md:flex-row justify-between gap-6">
               <div className="space-y-4 flex-1">
                  ...
                  <div className="p-6 rounded-2xl bg-red-50 dark:bg-red-500/5 border border-red-100 dark:border-red-500/10 italic text-red-700 dark:text-red-400 font-medium">
                     "{dispute.reason}"
                  </div>
               </div>
               ...
               <div className="flex flex-col gap-3 min-w-[200px]">
                  <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 space-y-1">
                     <p className="text-[10px] font-black text-primary uppercase tracking-widest">Reported By</p>
                     <p className="font-bold text-black dark:text-white">{dispute.user?.name}</p>
                  </div>
                  {dispute.status === "RESOLVED" && (
                    <div className="p-4 rounded-2xl bg-green-500/5 border border-green-500/10 space-y-1">
                       ...
                       <p className="text-sm font-bold text-green-700 dark:text-green-400">Resolved by Admin</p>
                    </div>
                  )}
               </div>
            </div>
        ...
        {disputes.length === 0 && (
          <div className="py-20 text-center space-y-4 bg-gray-50 dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-white/5">
             ...
             <div>
                <h3 className="text-2xl font-black text-black dark:text-white">Clean Record</h3>
                <p className="text-gray-400 font-bold">You have no reported disputes on your tasks.</p>
             </div>
          </div>
        )}
      </div>
